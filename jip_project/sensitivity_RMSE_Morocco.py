...
# Read the Excel file
excel_file = r'heliostrome\jip_project\results\sensitivity_Morocco.xlsx'
sheet_name = "soil_morocco"
...
# def calculate_rmse(average_yields_list, experimental_yields_list):
#    if len(average_yields_list) != len(experimental_yields_list):
#        raise ValueError("Input lists must have the same length")

#    squared_errors = [(actual - predicted) ** 2 for actual, predicted in zip(average_yields_list, experimental_yields_list)]
...
# Calculate RMSE
# rmse = calculate_rmse(average_yields_list, experimental_yields_list)
# print("RMSE:", rmse)
...
def calculate_rmse_percentage(average_yields_list, experimental_yields_list):
    ...
    denominator = np.mean(experimental_yields_list)
...
rmse_percentage = calculate_rmse_percentage(average_yields_list, experimental_yields_list)
