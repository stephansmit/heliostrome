...
rmse_df = pd.DataFrame(columns=['soil type', 'Yield RMSE', 'Yield RMSE Percentage', 'Water Used RMSE', 'Water Used RMSE Percentage'])
...
for input_sheet_name, output_sheet_name in zip(input_sheet_names, output_sheet_names):
    ...
    # ouput
    df_to_append = pd.DataFrame({'soil type': soil_type, 'Yield RMSE': yield_rmse, 'Yield RMSE Percentage': yield_rmse_percentage, 'Water Used RMSE': water_used_rmse, 'Water Used RMSE Percentage': water_used_rmse_percentage}, index=[0])
