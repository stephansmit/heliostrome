...
# 定义输入和输出文件的路径
input_file_path = r'heliostrome/jip_project/results/sensitivity_Morocco_InitialWaterContent.xlsx'
output_file_path = r'heliostrome\jip_project\results\sensitivity_RMSE_Morocco_InitialWC.xlsx'
...
# 获取所有输入和输出表的名称
input_sheet_names = [f"{InitialWC_type}_Input Parameters" for InitialWC_type in InitialWC_types]
output_sheet_names = [f"{InitialWC_type}_Output Results" for InitialWC_type in InitialWC_types]

# 创建一个新的 Excel writer 来保存 RMSE 结果
writer = pd.ExcelWriter(output_file_path, engine='openpyxl')

# 创建一个空的 DataFrame 用于保存 RMSE 结果
rmse_df = pd.DataFrame(columns=['InitialWC type', 'RMSE'])
...
for input_sheet_name, output_sheet_name in zip(input_sheet_names, output_sheet_names):
    ...
    # 获取土壤类型名称
    soil_type = input_df['InitialWC Type'].iloc[0]

    # 将 RMSE 结果添加到 rmse_df 中
    rmse_df = rmse_df.append({'InitialWC type': soil_type, 'RMSE': rmse}, ignore_index=True)
