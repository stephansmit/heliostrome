...
writer1 = pd.ExcelWriter(
    r"heliostrome\jip_project\results\WaterFlux_Bangladesh.xlsx", engine="openpyxl"
)


for i in range(len(extracted_rows["Case Study"])):
    location = Location(
        latitude=extracted_rows["Latitude"][i], longitude=extracted_rows["Longitude"][i]
    )
    ...
    irr_mngt = IrrigationManagement(
        irrigation_method=1, smt=extracted_rows["Irrigation Method"][i]
    )
...
writer = pd.ExcelWriter(
    r"heliostrome\jip_project\results\test_results_Bangladesh.xlsx", engine="openpyxl"
)
...
output_path_analyse = (
    r"heliostrome/jip_project/results/analysed_WaterFlux_Bangladesh.xlsx"
)
clean_excel_file(
