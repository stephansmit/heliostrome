...
writer = pd.ExcelWriter(
    r"heliostrome\jip_project\results\sensitivity_Morocco_crop.xlsx", engine="openpyxl"
)
...
for crop_type in crop_types:
    ...
    for i in range(len(extracted_rows["Case Study"])):
        ...
        print(
            f"Iteration {i+1} (Crop Type: {crop_type}): Sowing Date = {sowing_date}, Start Date = {start_date}, End Date = {end_date}"
        )
        start_time = end_time
    ...
    final_input_df.to_excel(
        writer, index=False, sheet_name=crop_type + "_Input_Parameters"
    )
