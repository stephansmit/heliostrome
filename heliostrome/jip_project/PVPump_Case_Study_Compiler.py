...
from modules.Pump_module import (
    convert_Qlpm,
    pump_solar_voltage_current_plot,
    pump_solar_voltage_power_plot,
)
...
# Load the Excel file
sheet_name = (
    "Case study pump 2"  # Replace with the name of the sheet containing the data
)
extracted_rows = factors_to_run(
    sheet_name
...
for i in range(len(extracted_rows["Case Study"])):
    ...
    pvgen1 = pvgen.PVGeneration(
        ...
        pv_module_name=extracted_rows["PV Module Name"][
            i
        ],  # Name of pv module to model
    ...
    voltage_solar, current_solar = get_iv_curve_solar(
        pvps1.pvgeneration.system, 1000, 25
    )

    pump_df = pd.DataFrame(voltage_pump, columns=["Voltage Pump"])
    pump_df["Current Pump"] = current_pump
    pump_df["Case Study"] = [
        extracted_rows["Case Study"][i] for _ in range(len(pump_df))
    ]
    solar_df = pd.DataFrame(voltage_solar, columns=["Voltage Solar"])
    solar_df["Current Solar"] = current_solar
    solar_df["Case Study"] = [
        extracted_rows["Case Study"][i] for _ in range(len(solar_df))
    ]
...
PVPump_results_df.index = PVPump_results_df.index.map(
    lambda x: x.replace(year=sim_start_year)
)
...
writer = pd.ExcelWriter(
    r"heliostrome\jip_project\results\PVPUmp_Data.xlsx", engine="openpyxl"
)
