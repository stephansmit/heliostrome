...
output_file_path = r'heliostrome\jip_project\results\sensitivity_Mean_Morocco_soil.xlsx'
...
for input_sheet_name, output_sheet_name in zip(input_sheet_names, output_sheet_names):
    ...
    yield_a_mean = np.mean(matched_data['Yield (Ton/HA)'])
    
    # Water Used Mean
    water_used_mean = np.mean(matched_data['Seasonal irrigation (mm)'])
    
    # Water Use Actually
    water_mean = np.mean(matched_data['Water Used (mm)'])


    soil_type = input_df['Soil Type'].iloc[0]

    # ouput
    df_to_append = pd.DataFrame({'soil type': soil_type, 'Yield': yield_mean,'Yield Actually': yield_a_mean, 'Water Used': water_used_mean,'Water Used Actually':water_mean}, index=[0])
