...
def Precip_data(sheets_factors_to_run):
    ...
    for i in range(len(sheets_factors_to_run)):
        extracted_rows = factors_to_run(
            sheets_factors_to_run[i]
        )  # Data extracted from an Excel file containing all data to run ClimateData

        # Create a new Excel file for each sheet in sheets_factors_to_run
        excel_writer = pd.ExcelWriter(
            f"heliostrome/jip_project/results/precip_data_{sheets_factors_to_run[i]}.xlsx",
            engine="openpyxl",
        )
        ...
        for x in range(len(extracted_rows["Case Study"])):
            ...
            precip_df.to_excel(
                excel_writer, sheet_name=extracted_rows["Case Study"][x], index=False
            )
