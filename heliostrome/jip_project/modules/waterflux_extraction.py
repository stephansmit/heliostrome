...
def extract_rows(input_path, output_path, start_date=None, field_size=None):
    ...
    for sheet_name in xls.sheet_names:
        ...
        if start_date is not None:
            ...
            extracted_df["Date"] = pd.to_datetime(start_date) + pd.to_timedelta(
                extracted_df["time_step_counter"], unit="D"
            )
...
def clean_excel_file(input_path, output_path, start_date=None):
    ...
    for sheet_name in xls.sheet_names:
        ...
        if start_date is not None:
            # Add a new column with the calculated dates
            df["Date"] = pd.to_datetime(start_date) + pd.to_timedelta(
                df["time_step_counter"], unit="D"
            )
...
def min_max_irrigation(excel_file, field_size=None):
    ...
    for sheet_name in xls.sheet_names:
        ...
        result_df = result_df.append(
            {
                "variation": sheet_name,
                "max_irrigation": max_irrigation,
                "min_irrigation": min_irrigation,
            },
            ignore_index=True,
        )
...
def summarize_monthly_data(input_path, output_path):
    ...
    for sheet_name in xls.sheet_names:
        ...
        if "Date" in df.columns:
            ...
            monthly_summary = (
                df.groupby([df["Date"].dt.year, df["Date"].dt.month])["IrrDay"]
                .sum()
                .reset_index()
            )
