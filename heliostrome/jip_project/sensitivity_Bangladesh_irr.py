...
irr_values = np.arange(5, 60.0, 5)

# Create an Excel writer to save the results
writer = pd.ExcelWriter(r'heliostrome\jip_project\results\sensitivity_Bangladesh_irr.xlsx', engine='openpyxl')
...
for irr_value in irr_values:
    ...
    for i in range(len(extracted_rows["Case Study"])):
        ...
        irr_mngt = IrrigationManagement(irrigation_method=1, smt = irr_value)
        ...
        print(f"Iteration {i+1} (Irrigation Management: {irr_value}): Sowing Date = {sowing_date}, Start Date = {start_date}, End Date = {end_date}")
        start_time = end_time
    ...
    final_input_df.to_excel(writer, index=False, sheet_name=str(irr_value) + "_Input_Parameters")
    final_df.to_excel(writer, index=False, sheet_name=str(irr_value) + "_Output_Results")
